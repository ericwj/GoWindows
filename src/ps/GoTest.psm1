...
function ConvertTo-Go {
	...
	Process {
		...
		if (Get-IsNotEmpty [string]$InputObject.Arg1) {
		...
		if (Get-IsNotEmpty [string]$InputObject.Arg2) {
		...
		if (Get-IsNotEmpty [string]$InputObject.Arg3) {
		...
		if (Get-IsNotEmpty [string]$InputObject.Arg4) {
...
function ConvertTo-GoCsv {
	...
	Process {
		...
		if ($InputObject -is [hashtable]) {
			Write-Debug "Processing hashtable $InputObject"
			foreach ($key in $InputObject.Keys) {
				$value = $InputObject[$key]
				if (-not $value -is [string] -and $value -is [System.Collections.IEnumerable]) {
					if ($key -eq "Traits") { $value = $value -join "," }
					else { $value = $value -join ";" }
				}
				if ($null -eq $key) {
					Write-Verbose "Encountered `$null key in $InputObject"
				} else { $Result.Add($key, $value) }
			}
		} elseif ($InputObject -is [pscustomobject]) {
			Write-Debug "Processing pscustomobject $InputObject"
			foreach ($name in ($InputObject | Get-Member -Type NoteProperty).Name) {
				$value = $InputObject."$name"
				if (-not $value -is [string] -and $value -is [System.Collections.IEnumerable]) {
					if ($name -eq "Traits") { $value = $value -join "," }
					else { $value = $value -join ";" }
				} else {
					if ($null -eq $value) {
					...
					$Result | Add-Member -MemberType NoteProperty -Name $name -Value $value -TypeName $TypeName
				}
			}
		} else {
			Write-Warning "ConvertTo-GoCsv: Cannot handle type '$($InputObject.GetType())' of value '$InputObject'. Passing through unmodified."
		}
		$All += $Result
	}
...
function Remove-GoNulls {
	...
	Process {
		if ($null -eq $InputObject) {
			...
			foreach ($key in $keys) {
				$value = $InputObject[$key]
#				Write-Debug "Evaluating $key=$value"
				if (Get-GoIsDefault -InputObject $value @Splat) {
			...
			foreach ($name in ($InputObject | Get-Member -Type NoteProperty).Name) {
				$value = $InputObject."$name"
#				Write-Debug "Evaluating $name=$value"
				if (Get-GoIsDefault -InputObject $value @Splat) {
